...
using Microsoft.AspNetCore.Mvc.Testing; // Wichtiges neues Using!
using Xunit;
...
namespace ChessServer.Tests
{
    ...
    public class GamesControllerTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        public async Task MakeMoveWhenCalledInParallelForSameTurnOnlyOneSucceeds()
        {
            ...
            var createResponse = await _client.PostAsJsonAsync("/api/games", createDto);
            ...
            var createResult = await createResponse.Content.ReadFromJsonAsync<CreateGameResultDto>();
            ...
            var joinResponse = await _client.PostAsJsonAsync($"/api/games/{gameId}/join", joinDto);
            ...
            var task1 = _client.PostAsJsonAsync($"/api/games/{gameId}/move", moveDto);
            var task2 = _client.PostAsJsonAsync($"/api/games/{gameId}/move", moveDto);
            ...
            {
                var errorBody = await failedRequest.Content.ReadFromJsonAsync<MoveResultDto>();
